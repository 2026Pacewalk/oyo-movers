...
const OnePageBookingPage = async ({ searchParams }: any) => {
  ...
  if (searchParams?.payment_intent) {
    ...
    if (res?.status === 200) {
      ...
      redirect(`/one-page-booking/confirmation?${confirmParams.toString()}`);
    } else {
      isPaymentDone = false;
    }
  }
  ...
  return (
    <div className={`${inter.className} one-page-booking-page`}>
      ...
      {searchParams?.payment_intent && (
        <CheckPayment
          ...
          confirmationPath="/one-page-booking/confirmation"
        />
      )}
